#!/bin/sh
#./GenAnalyzer/GenNtuplizer/scripts/process_all.sh /eos/cms/store/cmst3/group/taug2/MiniAOD/GGToTauTau_Elastic_M_50GeV_PTFilter_ktSmear_RunIISummer20UL18_TuneCP5_madgraphLO_reweight/
...
cp $CMSSW_BASE/src/GenAnalyzer/GenNtuplizer/scripts/run_job.sh .
...
for f in `ls ${sourceFolder}/*root`; do
  ...
  if [ "$DOCONDOR" -eq 1 ]; then
    ...
    echo process $f
    source $CMSSW_BASE/src/GenAnalyzer/GenNtuplizer/scripts/run_job.sh $f $outfile $CMSSW_BASE
  fi
